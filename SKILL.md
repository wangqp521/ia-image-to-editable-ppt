...
默认采用草稿优先的 `rapid`：先生成并结构验证 PPTX，LibreOffice 只做一次可选、非阻断预览。字体回退、LibreOffice `SIGABRT`、缺少 Poppler 或预览失败都不得阻止交付已通过结构验证的可编辑草稿。
...
- `reviewed`：用户明确要求独立复核时使用。保留严格 runtime、render、visual diff、reviewer 与 final 身份闭包；reviewer 最多两轮。

`rapid` 中，结构、内容完整性或主要内容可编辑性失败才阻断交付。预览不可用或字体回退时写 `rapid_validation_failed` 并交付草稿；结构及语义判断通过且没有开放 P0/P1 时可写 `rapid_validated`。不得把草稿称为独立复核通过版。
...
结构与背景通过后，PPTX 已满足草稿交付条件。只有需要视觉预览时才运行一次：

```bash
python3 scripts/render_preview.py work/page.pptx --preferred-font "Hiragino Sans GB" --output-dir preview/PPTX_SHA256
```

macOS 必须从第一次就把 LibreOffice 放在允许启动应用的执行环境中运行；脚本使用独立可写 profile 和进程锁。`rapid` 直接预览只尝试一次：任何 command error、`SIGABRT`、无 PDF、字体 mismatch 或 Poppler 缺失都记录为预览不可用/字体回退，不重试、不运行 visual diff，继续交付结构通过的 PPTX。

若预览成功，主代理核对整页 mapping、层级、文字、换行、图形、图表、crop、图标与背景，一次列全 P0/P1；同根因问题合并为一次集中修复。修复后从 prebuild 起重建，并重新执行结构、背景及最多一次预览。字体回退不是修复理由。

## reviewed 严格链

只有用户明确要求 `reviewed` 时才运行 `preflight_runtime.py`，并在 prebuild/render 传入 `--runtime`。继续生成 visual diff、reviewer prompt 与只读 final；固定字体、runtime 和证据身份仍按严格合同处理。详细规则见[视觉审计与交付](references/visual-audit-and-delivery.md)。严格链失败时仍交付已经生成且结构通过的当前 PPTX，但不得称 reviewed 通过。

## 多页与交付

逐页执行。单页 prebuild/build/结构失败时保留诊断并继续后页；结构通过的页面可进入草稿合并，不要求 runtime、preview 或 final report：
...
草稿 merger 会重新检查每个输入是结构有效的单页 16:9 PPTX，并在合并后验证整份 deck。交付可编辑 PPTX、结构报告以及实际存在的预览/诊断；明确披露缺页、开放 P0/P1、字体回退与未验证项。不得因缺少 LibreOffice 证据隐藏或扣留已通过结构验证的草稿。
