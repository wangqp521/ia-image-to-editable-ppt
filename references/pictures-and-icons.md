...
`extract_assets.py` 是 schema v2 素材批处理编排器：普通 picture 支持精确 `source_patch`、显式 `background_preserved` 和 `explicit_mask`；图标的 `alpha_isolation/background_preserved` 委托 `extract_icon_asset.py`，不复制算法。非 icon 请求自动 alpha 且没有显式 mask 时固定返回 `ALPHA_EXTRACTION_UNSAFE`。全部任务先写临时目录，全部成功才发布素材并一次回填路径、尺寸、asset/alpha hash；失败不得污染规格或留下部分资产。统一用 `create_asset_crop_review.py` 查看 context/source/绿色背景 asset；`assets-report.json` 只是执行证据。

## 图标

图标必须从当前页视觉参考裁切为各自独立 picture，放回原 bbox；简单或复杂图标都不得用 Shape、字符、重绘 SVG 或图标库替代。编号圆点、标签、文字、bullet 和分隔线不是图标，不得混入 `icon_only`。`extract_icon_asset.py` 是唯一图标资产生成入口，每次只处理一个已测量图标；批量入口只负责委托和证据汇总，不得编写页面专用裁切脚本、增加对象检测状态机或第二套资产规格。
...
全部素材写入路径/hash 后、prebuild 前统一运行 `create_asset_crop_review.py`。图标按 `icon_id` 展示“带 bbox 的局部上下文 / source/asset 400%”，非图标以 200%–300% 查看 crop/mask/alpha，资产侧固定使用 `#00FF00` 绿幕。统一 `asset_manifest_sha256` 绑定真实源图、元素类型与顺序、`source_bbox`、padding、crop mode、真实资产 SHA-256、背景处理/fallback、alpha mask 和固定渲染参数；命中 PNG metadata 时返回 `reused=true`。`spec_sha256` 绑定当前 prebuild 报告但不参与 PNG 缓存，非素材字段变化不得使旧绿幕复核图失效。真实源图或任一素材依赖变化、metadata 缺失/损坏时必须重建；缓存命中也不得跳过真实输入校验。旧 `create_icon_crop_review.py` 仅兼容历史任务，新任务不得双重生成。

通过 commentary 以 `[第 N/总页数] 图标裁切绿幕复核` 展示当前统一素材复核图；无图标时不生成也不展示图标栏，但有其他图片仍展示素材复核。缓存不替代首次展示和审查。放大只供审查，不写回资产或 PPTX；先确认 ROI 语义正确，再确认轮廓完整、背景处理正确、开放线框/封闭区符合连通规则且没有白边、色晕或漏裁，才运行 prebuild。bbox 不足先扩大重跑；仅透明结果本身损坏时才改用 `background_preserved`。PPTX 渲染后仍以 `placement_400` 检查位置、比例、清晰度、裁剪和局部底色接缝；三种 profile 都必须完成。
...
border/shadow/reflection/glow 只在来源可见时使用，分别记录方向、距离、blur、透明度、颜色、扩散和层级；不得套默认效果。统一 builder 已支持焦点 cover、显式 crop 和 rotation；尚未实现的 mask/effect 必须返回 `UNSUPPORTED_FEATURE`，由主代理选择可验证原生实现或最小局部 picture，不得静默丢弃。以整页查构图、位置、焦点和层级，再以 200%–300% 查 crop/mask/alpha/圆角/边框/效果。误裁、拉伸、边缘断裂、拼接缝或效果方向错误不得通过。
