...
如果已有结构有效的 PPTX，但视觉验证未通过，将保留当前产物并明确标注；如果构建或编译失败，则可能只交付诊断证据，不生成 PPTX。Skill 不会偷偷降低验证标准或伪造通过状态。
...
- 同一批页面不能混用验证模式；
- 如果已有结构有效的 PPTX，但视觉验证未通过，将交付明确标注的当前产物；如果构建或编译失败，则可能只交付诊断证据，不生成 PPTX，且不会自动降低标准。
